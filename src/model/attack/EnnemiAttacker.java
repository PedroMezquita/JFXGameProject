...
public class EnnemiAttacker implements Attacker{
    ...
    public Attack attack(Personnage pers, Direction dir) {
        if (dir.getyDir() == 1){
            ...
            return new Attack(pers.getAttaque().getLifeTime(), pers.getAttaque().getDegats()*pers.getAtkPnt(), atqXPos, atqYPos, pers.getAttaque().getxSize(), pers.getAttaque().getySize(), pers.getAttaque().getId(), pers, dir);
        }
        if (dir.getyDir() == -1){
        ...
        if (dir.getxDir() == -1){
            ...
            return new Attack(pers.getAttaque().getLifeTime(), pers.getAttaque().getDegats()*pers.getAtkPnt(), atqXPos, atqYPos, pers.getAttaque().getySize(), pers.getAttaque().getxSize(), pers.getAttaque().getId(), pers, dir);
        }
        if (dir.getxDir() == 1){
